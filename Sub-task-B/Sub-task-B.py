...
import time
import helperfunction as hfunc
  ...
  for i in os.listdir(inp_dir + folder + '/'):
    ...
    for files in os.listdir(inp_dir + folder + '/' + str(i)):
      if files.endswith("Stanza-out.txt"):
        stanza_file = open(inp_dir + folder + '/' + str(i) + '/' + files, "r")
        stanza_lines = (stanza_file.read()).lower()
        stanza_lines_list = list(filter(None,stanza_lines.splitlines())) 
        in_stanza_list.append(stanza_lines_list)
      if files.endswith("sentences.txt"):
        sentence_file = open(inp_dir + folder + '/' + str(i) + '/' + files, "r")
        sentence_num_list = list(filter(None,(sentence_file.read().lower()).splitlines())) 
        in_sent_numbers.append(list(map(int, sentence_num_list)))
      if files.endswith("entities.txt"):
        entities_file = open(inp_dir + folder + '/' + str(i) + '/' + files, "r")
        entities_list = list(filter(None,(entities_file.read().lower()).splitlines())) 
...
valid_list_of_folders = ["machine-translation", "named-entity-recognition", "question-answering", "relation-classification", "text-classification"] 
...
for folder in valid_list_of_folders:
  ...
  for i in os.listdir(valid_inp_dir + folder + '/'):
    ...
    for files in os.listdir(valid_inp_dir + folder + '/' + str(i)):
      if files.endswith("Stanza-out.txt"):
        stanza_file = open(valid_inp_dir + folder + '/' + str(i) + '/' + files, "r")
        stanza_lines = stanza_file.read().lower()
        stanza_lines_list = list(filter(None,stanza_lines.splitlines())) 
        valid_in_stanza_list.append(stanza_lines_list)
      if files.endswith("sentences.txt"):
        sentence_file = open(valid_inp_dir + folder + '/' + str(i) + '/' + files, "r")
        sentence_num_list = list(filter(None,(sentence_file.read().lower()).splitlines())) 
        valid_in_sent_numbers.append(list(map(int, sentence_num_list)))
      if files.endswith("entities.txt"):
        entities_file = open(valid_inp_dir + folder + '/' + str(i) + '/' + files, "r")
        entities_list = list(filter(None,(entities_file.read().lower()).splitlines())) 
        valid_in_entities.append(entities_list)
        valid_total_phrases_truth = valid_total_phrases_truth + len(entities_list)
  ...
  try:
    ...
    for n,ind_s,ind_e,ph in entity_list: 
        if int(n) in sent_num_list:
            sent_dict_list[int(n)] = sent_dict_list[int(n)].replace(ph,hfunc.BILOU_substring(len(ph.split())),1)
...
for i in range(len(valid_in_stanza_list)): 
  ...
  for n ,ind_s ,ind_e, ph in valid_entity_list: 
    if int(n) in valid_sent_num_list:
      valid_sent_dict_list[int(n)] = valid_sent_dict_list[int(n)].replace(ph,hfunc.BILOU_substring(len(ph.split())),1)
...
for i,out in enumerate(valid_taskB_label): 
  for j,line in enumerate(out):
    for k,tok in enumerate(line.split()):
      if tok not in ['B','I','L','U']:
        valid_taskB_label[i][j]=valid_taskB_label[i][j].replace(tok,'O',1)
...
class BiLSTM_CRF(nn.Module):
    ...
    def _forward_alg(self, feats):
        ...
        for feat in feats:
            ...
            for next_tag in range(self.tagset_size):
                ...
                alphas_t.append(hfunc.log_func(next_tag_var).view(1))
            forward_var = torch.cat(alphas_t).view(1, -1)
        terminal_var = forward_var + self.transitions[self.biluo_code[STOP_TAG]]
        alpha = hfunc.log_func(terminal_var)
        return alpha
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = torch.cat([torch.tensor([self.biluo_code[START_TAG]], dtype=torch.long).to(device), tags])
    ...
    def _viterbi_decode(self, feats):
        ...
        for feat in feats:
            ...
            for next_tag in range(self.tagset_size):
                ...
                best_tag_id = hfunc.maxval(next_tag_var)
        ...
        terminal_var = forward_var + self.transitions[self.biluo_code[STOP_TAG]]
        best_tag_id = hfunc.maxval(terminal_var)
...
biluo_code = {"B": 0, "I": 1, "L": 2, "U": 3, "O": 4, START_TAG: 5, STOP_TAG: 6}
...
def train_eval():
  ...
  for i,file in enumerate(taskB_in):
    ...
    for j,sent in enumerate(file):
      with torch.no_grad():
        precheck_sent = hfunc.create_input(taskB_in[i][j].split(), tokenizer).to(device)
  ...
  for i,file in enumerate(taskB_label):
    file_true_pos, file_total_ph_pred = eval_func(taskB_out_label[i],[["O"] + s.split() + ["O"] for s in taskB_label[i]])
...
def validation_eval():
  ...
  for i,file in enumerate(valid_taskB_in):
    ...
    for j,sent in enumerate(file):
      with torch.no_grad():
        precheck_sent = hfunc.create_input(valid_taskB_in[i][j].split(), tokenizer).to(device)
        sent_out_label = model(precheck_sent)[1]
        sent_str_label = [biluo_decode[t] for t in sent_out_label]
        output.append(sent_str_label)
  ...
  for i,file in enumerate(valid_taskB_label):
    valid_file_true_pos, valid_file_total_ph_pred = eval_func(valid_taskB_out_label[i],[["O"] + s.split() + ["O"] for s in valid_taskB_label[i]])
  ...
  if((valid_precision + valid_recall)!=0):  
    valid_F1score = 2 * valid_precision*valid_recall/(valid_precision+valid_recall)
  print("Precision : {} | Recall : {} | F1 Score : {}".format(valid_precision,valid_recall,valid_F1score))
...
with torch.no_grad():
    ...
    precheck_sent = hfunc.create_input(taskB_in[0][0].split(), tokenizer).to(device) 
    precheck_tags = torch.tensor([4] + [biluo_code[t] for t in taskB_label[0][0].split()] + [4], dtype=torch.long).to(device) 
    ...
    for i,file in enumerate(taskB_in):
      try:
        ...
        for j,sent in enumerate(file):
            ...
            sentence_in = hfunc.create_input(sent.split(), tokenizer).to(device)
    
            targets = torch.tensor([4] + [biluo_code[t] for t in taskB_label[i][j].split()] + [4], dtype=torch.long).to(device)
    ...
    with torch.no_grad():
      for k, valid_file in enumerate(valid_taskB_in):
        for l, valid_sent in enumerate(valid_file):
          valid_sentence_in = hfunc.create_input(valid_sent.split(), tokenizer).to(device)
          valid_targets = torch.tensor([4] + [biluo_code[t] for t in valid_taskB_label[k][l].split()] + [4], dtype=torch.long).to(device)
          loss = model.neg_log_likelihood(valid_sentence_in, valid_targets)
...
for folder in test_list_of_folders:
  ...
  for i in os.listdir(test_inp_dir + folder + '/'):
    ...
    for files in os.listdir(test_inp_dir + folder + '/' + str(i)):
      if files.endswith("Stanza-out.txt"):
        stanza_file = open(test_inp_dir + folder + '/' + str(i) + '/' + files, "r")
        print(test_inp_dir + folder + '/' + str(i))
        Capital_stanza_lines = stanza_file.read()
        Capital_stanza_lines_list = list(filter(None,Capital_stanza_lines.splitlines())) 
      ...
      if files.endswith("sentences.txt"):
        sentence_file = open(test_inp_dir + folder + '/' + str(i) + '/' + 'sentences.txt', "r")
        sentence_num_list = list(filter(None,(sentence_file.read().lower()).splitlines())) 
...
for i,file in enumerate(test_taskB_in):
  ...
  for j,sent in enumerate(file):
    with torch.no_grad():
      precheck_sent = hfunc.create_input(test_taskB_in[i][j].split(), tokenizer).to(device)
      sent_out_label = model(precheck_sent)[1]
      sent_str_label = [biluo_decode[t] for t in sent_out_label]
      output.append(sent_str_label)
...
for i,file in enumerate(test_taskB_in):
  ...
  for j,sent in enumerate(file):
    ...
    sentence_number = (list_of_dict_for_sentence_to_number[i])[test_taskB_in[i][j]]
    ...
    for k in zip(biluo_list,respective_sentence):
      ...
      if (k[0]=="U"): 
        ...
        if(cnt_of_words_in_sentence == 0):
          ...
          start_of_word = len((" ".join(respective_sentence[0:cnt_of_words_in_sentence])).strip() + " ") 
        ...
        end_of_words = len((" ".join(respective_sentence[0:cnt_of_words_in_sentence])).strip() + " ") + len(respective_sentence[cnt_of_words_in_sentence].strip())
